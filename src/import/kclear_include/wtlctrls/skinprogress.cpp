...
BOOL CSkinProgress::SubclassWindow(
	...
	UINT unBmpID)
{
	BOOL bRetCode = SKIN_PROGRESS_BASE::SubclassWindow(hWnd);
	if (bRetCode == FALSE)
		return FALSE;
...
BOOL CSkinProgress::GetTimed(
	ATL::CString *o_poTimed)
{
	ATL::CString oStrTempo;
	...
	CDCHandle dc) 
{
	...
	hOldBitmap = MemDC.SelectBitmap(*mpProgressBmp);
	...
	PaintDC.SelectFont(gGuiFont.mFont);
...
void CSkinProgress::OnTimer(
	UINT nIDEvent)
{
	if (nIDEvent == (UINT)(ULONG_PTR)this)
	{
		OnCalcPos();
	}
}
