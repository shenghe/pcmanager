...
class CDropArrowsEx : public CWindowImpl< CDropArrowsEx >
{
public:
	CDropArrowsEx()
	{
...
public:
	BOOL Create( HWND hWndParent, int nSpanLength, BOOL bVertical )
	{
		...
		if ( CWindowImpl< CDropArrowsEx >::Create( hWndParent, CRect( 0, 0, m_bVertical ? 12 : m_nSpanLength, m_bVertical ? m_nSpanLength : 12 ), NULL, WS_POPUP | WS_DISABLED, WS_EX_TOOLWINDOW ) == NULL )
			return FALSE;
