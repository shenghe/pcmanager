...
#define CALC_HASH(fileobj)  ((ULONG)(fileobj % HASH_SIZE))
#pragma pack(push , 1)
...
template< typename K, typename T , BOOLEAN LOCK = TRUE ,int HASH_SIZE = 1000 >
class KWDMMap
{
public:
	typedef struct _Hash_Node<K,T> Hash_Node;
	typedef struct _Hash_Node<K,T>* PHash_Node;
	struct Iterator
	{
...
public:
	...
	*/
	...
	BOOLEAN insert(K ulKey, T ulPtrValue)
	{
		KIRQL oldirq;
...
EXIT0:	
	...
	BOOLEAN erase_nolock(K ulKey, T* tValue)
	{
		PHash_Node pEntry = NULL;
	...
	BOOLEAN erase(K ulKey, T* tValue)
	{
		KIRQL oldirq;
		BOOLEAN bFind = FALSE;
	...
	BOOLEAN find_lock(K ulKey ,  T* tValue)
	{
		KIRQL oldirq;
		BOOLEAN bFind = FALSE;
		KeAcquireSpinLock(&m_spinLock , & oldirq);
		...
		for ( int i = 0 ; i < HASH_SIZE ; i++ )
		{
			...
			{			
				...
				if( bFreeValue )
				{
					...
					non_paged_pool::__DeallocateSystemMemory( (PVOID)pFree->tValue );
					pFree->tValue = NULL;
				}
