...
#else
#define CURL_EXTERN  //__declspec(dllimport) //comment this to avoid the link warning LNK4217 (see MSDN for more) (zhangzhou, April, 3, 2006)
#endif
#else
#define CURL_EXTERN
#endif
...
typedef enum {
  ...
  CURLE_PROXY_AUTHORIZATION_FAILED, /* 75 - Proxy user name or password incorrect */
  CURL_LAST /* never use! */
} CURLcode;
...
typedef enum {
  ...
  CURLINFO_CONNECT_IP_ADDR  = CURLINFO_STRING + 29,
